...
/**
 * Gets allowed file types for Tifinagh
 * 
 * @access      private
 * @since       1.3
 * @return      array
...
function fu_upload_font_file() {
	if( isset( $_POST['fu_action'] ) && $_POST['fu_action'] == 'upload' ) {
		...
		if( ! empty( $font['error'] ) )
			wp_die( $font['error'], __('Font Upload Failure', 'tifinagh') );

		wp_redirect( add_query_arg( 'font', 'uploaded', admin_url( 'themes.php?page=tifinagh' ) ) ); exit;
	}
}
...
function fu_admin_notices() {
	...
	if( $pagenow == 'themes.php' && isset( $_GET['font'] ) && $_GET['font'] == 'uploaded' ) {
		echo '<div class="updated"><p>' . __('Your font have been successfully uploaded', 'tifinagh') . '</p></div>';
	}
}
