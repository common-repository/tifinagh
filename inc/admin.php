...
function fu_admin_page() {
	add_theme_page( __('Tifinagh', 'tifinagh'), __('Tifinagh', 'tifinagh'), 'manage_options', 'tifinagh', 'fu_render_admin');
}
...
function fu_render_admin() {
    ...
    if ( isset( $_REQUEST['saved'] ) && $_REQUEST['saved'] ) echo '<div id="message" class="updated fade"><p><strong>Tifinagh settings saved.</strong></p></div>';
	?>
	<div class="wrap">
		
		<h2>Tifinagh</h2>
		<p>Uploaded Tifinagh fonts will appear in the menus below</p>	
		<form method="post" enctype="multipart/form-data" action="themes.php?page=tifinagh">
			...
			<div class="description"><em><?php _e('Filetypes accepted: ', 'tifinagh'); ?><strong>.ttf</strong>, and <strong>.otf</strong></em></div>
			<?php echo submit_button(__('Upload Font File', 'tifinagh') ); ?>
		</form>
	    ...
	    <form method="post">
	        ...
	        <?php echo submit_button(__('Save Changes', 'tifinagh') ); ?>
	    </form>
	</div>
	<?php
}
...
function fu_setup_options() {
	...
	$options = array(
	    ...
	    array( "name" => __('Font Upader', 'tifinagh'),
	        "type" => "title"),
...
function fu_save_options() {
    ...
    if ( isset( $_GET['page'] ) && $_GET['page'] == 'tifinagh'  && $pagenow == 'themes.php') {
